  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Symptom Assessment</h2>
        <p className="text-gray-600 dark:text-gray-300">Please select your primary symptoms and provide additional details</p>
      </div>
      
      <div>
        <label className="block text-lg font-medium text-gray-700 dark:text-gray-300 mb-4">
          Primary Symptoms <span className="text-sm text-gray-500">(Select all that apply)</span>
        </label>
      ...
      <div className="flex items-center justify-between">
        <button onClick={()=>setStep(2)} className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">← Back</button>
        <button onClick={()=>{ setSelectedSymptoms(selected); setSymptomDetails({ severity, duration, details }); setStep(4); }} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">Next →</button>
      </div>
    </div>
  );
}
