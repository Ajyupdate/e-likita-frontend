  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('consultation.steps.followups.title')}</h2>
        <p className="text-gray-600 dark:text-gray-300">{t('consultation.steps.followups.description')}</p>
      </div>
      
      {followups.length === 0 ? (
        <div className="text-center py-8">
          <div className="text-4xl mb-4">❓</div>
          <p className="text-gray-500 dark:text-gray-400">{t('consultation.steps.followups.noQuestions')}</p>
        </div>
      ) : (
        followups.map((f, idx) => (
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-0 pt-4">
        ...
        >
          ← {t('consultation.steps.followups.buttons.back')}
        </button>
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <span className="hidden xs:inline">{t('consultation.steps.followups.processing')}</span>
              <span className="xs:hidden">{t('consultation.steps.followups.processing')}</span>
            </>
          ) : (
            <>
              <span className="hidden sm:inline">{t('consultation.steps.followups.completeAssessment')} →</span>
              <span className="sm:hidden">{t('consultation.steps.followups.completeAssessment')}</span>
            </>
          )}
        </button>
      </div>
